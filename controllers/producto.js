...
const obtenerProductoPorNombre = async (req = request, res = response) => {
    ...
    const producto = await Producto.findOne({nombre}).populate('usuario', 'nombre').populate('categoria', 'nombre');
...
const obtenerProductoPorCategoria = async (req = request, res = response) => {
    ...
    const producto = await Producto.findOne({categoria})
        .populate('usuario', 'nombre')
        .populate('categoria', 'nombre');
    ...
    res.json({
        msg: 'producto de categoria por id',
        producto
    });
